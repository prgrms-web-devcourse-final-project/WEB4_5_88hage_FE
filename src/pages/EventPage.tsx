...
export default function EventPage() {
  ...
  const handleEventRecommend = async (
    address: string,
    start: string,
    end: string,
  ) => {
    ...
    try {
      ...
      setRecommendReasons(
        (json.data.events || []).map(
          (e: EventApiResponse) => e.reason || '추천 이유 없음',
        ),
      );
      setRecommendClick(0);
    } catch (e) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (type) {
        list = list.filter(
          (event: EventApiResponse) => event.eventType === type,
        );
      }
      setData((prev) => (page === 0 ? list : [...prev, ...list]));
      setHasMore(
        res.data && typeof res.data.last !== 'undefined'
          ? !res.data.last
          : false,
      );
      setLoading(false);
  ...
  return (
    <div className="w-full">
      ...
      <div className="meetingPage-gradient h-fit pt-[70px] pb-[25px] lg:h-[350px] lg:pt-[115px]">
