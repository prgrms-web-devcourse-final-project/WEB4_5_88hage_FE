...
export const createGroupHashtag = async (
  ...
  const res = await post<GroupHashtag>('/group-hashtags', data);
  return res;
};

// 모든 해시태그 목록 조회
export const getAllGroupHashtags = async (): Promise<GroupHashtag[]> => {
  const res = await get<GroupHashtag[]>('/group-hashtags');
  return res;
};
...
export const updateGroupHashtag = async (
  ...
  const response = await put<GroupHashtag>(`/group-hashtags/${id}`, data);
  return response;
};
...
export const getGroupHashtagById = async (
  ...
  const response = await get<GroupHashtag>(`/group-hashtags/${id}`);
  return response;
};
