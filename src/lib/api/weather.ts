...
const getWeather = async () => {
  ...
  try {
    const { data } = await axios.get(
      `http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtNcst?serviceKey=${weatherApiKey}&numOfRows=13&pageNo=1&base_date=${todayStringInfo}&base_time=${nowStringInfo}&nx=62&ny=125&dataType=JSON`,
    );
    const item = data.response.body.items.item;
    const precipitation = item.filter(
      (info: WeatherInfo) => info.category === 'PTY',
    );

    if (precipitation[0].obsrValue === undefined)
      throw new Error('api 형식을 참고 해주세요');
